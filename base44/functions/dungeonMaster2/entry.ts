...
`;
    ...
    const actionBlock = (is_roll_result && !isPJ)
...
Respond as the ${isNonDnd ? 'Game Master' : 'DM'} with the JSON object. Resolve the action using ${rulesLabel} rules. ${(is_roll_result && !isPJ) ? 'Continue the scene based on the roll outcome above.' : 'If this is the very first action and the scene is empty, open the campaign with atmospheric scene-setting narration that hooks the party into the adventure.'}`;
