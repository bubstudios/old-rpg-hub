...
Deno.serve(async (req) => {
  try {
    ...
    // Dashboard: all the user's campaigns, their characters, and featured shared adventures
    if (op === 'dashboard') {
      const myChars = await admin.entities.Character.filter({ created_by_id: user.id }, '-updated_date', 50);
      const createdCampaigns = await admin.entities.Campaign.filter({ created_by_id: user.id }, '-updated_date', 100);
      ...
      for (const ch of myChars) {
        ...
        try {
          const camp = await admin.entities.Campaign.get(ch.campaign_id);
      ...
      const featured = await admin.entities.AdventureModule.filter({ visibility: 'shared' }, '-updated_date', 6);

      return Response.json({ campaigns, characters, featured });
    }
    ...
    if (op === 'createCampaign') {
      ...
      const campaign = await base44.entities.Campaign.create({
        ...
        world_state: isPJCanon
          ? { locations_explored: [], npcs_met: [], quest_flags: { campaign_clocks: pjClocks, ship_stats: pjShipStats, evidence: ['Prometheus Warning','James Stellar Testimony','Korath Database','Novara Transaction Record','Sakura-Chen Technology Exchange','New Titan Claim File','Sarah Chen Testimony','Sanctuary Archive Records','Architect Future-History Data'], allies: ['Sarah Chen','James Stellar','Sanctuary refugee fleet','Councilor Verath','Commander Vex','37 allied ships','Mitchell'], enemies: ['The Confluence','Vescarri Sovereignty',"Collector's Guild",'Admiral Chen','Captain Vask','Confluence shapeshifters'], current_location: 'Edge of New Titan System', decisions: [] }, reputation: 0, chapter_log: [], ally_states: { sarah_chen: { state: 'TRUSTED', relationship: 45, need: 'Truth about her mother', last_action: 'Joined the Pathfinder crew' }, james_stellar: { state: 'LOYAL', relationship: 80, need: 'Purpose and redemption', last_action: 'Provided Confluence tactical knowledge' }, mitchell: { state: 'LOYAL', relationship: 75, need: 'Trust and respect', last_action: 'Sensed deception at Sanctuary' }, councilor_verath: { state: 'CAUTIOUS', relationship: 25, need: 'Civilian safety and proof', last_action: 'Granted limited archive access' }, commander_vex: { state: 'CAUTIOUS', relationship: 20, need: 'Strategy and military clarity', last_action: 'Committed defensive ships to the fleet' }, sanctuary_refugee_fleet: { state: 'CAUTIOUS', relationship: 35, need: 'Safety, fuel, and hope', last_action: '37 ships followed the Pathfinder from Sanctuary' }, '37_allied_ships': { state: 'CONDITIONAL', relationship: 30, need: 'Repairs, assignments, and morale', last_action: 'Joined the resistance fleet' } } }
