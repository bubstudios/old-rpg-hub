...
import {
  ChevronLeft, Heart, Flame, Coins, Package, Activity,
  Loader2, BookOpen, Download, Zap
} from 'lucide-react';
import { ABILITIES, WEAPON_SKILLS, getInitiativeMod, getMentationMod, bestWeaponSkillLevel } from '@/lib/hyRules';
import ExportCharacterButton from '@/components/ExportCharacterButton';
import { toast } from 'sonner';

export default function HyCharacterSheet({ character: initialCharacter, campaignId }) {
  ...
  if (!character) {
    return <div className="text-center py-20 text-muted-foreground">Warrior not found.</div>;
  }
  ...
  const menMod = getMentationMod(scores);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <StatBox icon={Heart} label="VITALITY" value={`${character.hp_current}/${character.hp_max}`} sub="Endurance" accent="red" />
        <StatBox icon={Zap} label="INIT" value={`${initMod >= 0 ? '+' : ''}${initMod}`} sub="AGI / 10" />
        <StatBox icon={Flame} label="MENTATION" value={`${menMod >= 0 ? '+' : ''}${menMod}%`} sub="Nerve Modifier" />
        <StatBox icon={Coins} label="GOLD" value={character.gold || 0} sub="Currency" />
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        ...
        <div className="border border-border/50 rounded-lg bg-card/40 p-4">
          ...
          <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-border/30">
            ...
            <span className="text-xs font-heading text-foreground">{character.gold || 0} gold</span>
          </div>
        </div>
