...
const STATUS_STYLES = {
  active: 'bg-emerald-200/50 text-emerald-800 border-emerald-600/30',
  paused: 'bg-red-200/50 text-red-800 border-red-600/30',
  completed: 'bg-emerald-200/50 text-emerald-800 border-emerald-600/30',
  setup: 'bg-amber-200/50 text-amber-800 border-amber-600/30',
};
      ...
      <div className="max-w-2xl mx-auto px-3 sm:px-4 py-6 sm:py-10">
        ...
        <div className="parchment-ornate rounded-xl p-5 sm:p-7">
          ...
          <div className="text-center mb-6">
            ...
            <p className="font-tome italic text-xs text-amber-900/50 mt-2">Your saga awaits</p>
          </div>
          ...
          {lastCampaign ? (
            ...
            >
              <div className="ornate-frame rounded-xl p-3 bg-gradient-to-br from-amber-950/10 to-amber-900/5 transition-all">
                <div className="flex items-center gap-3">
                  <div className="w-16 h-16 rounded-lg overflow-hidden shrink-0 border-2 border-amber-700/40 shadow-lg bg-amber-800/20">
                    <img src={thumbFor(lastCampaign.game_system)} className="w-full h-full object-cover" alt="" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-[9px] font-heading tracking-[0.2em] text-amber-700/70 uppercase mb-0.5">Resume Last Campaign</p>
                    <h2 className="font-heading font-700 text-base sm:text-lg text-amber-950 truncate group-hover:text-amber-800 transition-colors">{lastCampaign.name}</h2>
                    <div className="flex items-center gap-2 flex-wrap text-[10px] text-amber-900/50 font-body mt-0.5">
                      ...
                      {lastCampaign.combat_active && <span className="flex items-center gap-0.5 text-red-700"><Skull className="w-2.5 h-2.5" /> Combat</span>}
                    </div>
                  </div>
            ...
            <div className="mb-5 text-center py-6">
              <p className="font-tome italic text-sm text-amber-900/50">No campaigns yet — your saga awaits below.</p>
            </div>
          )}

          {/* Quick Actions — 3 parchment squares */}
          <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-6">
            {/* Join by Code */}
            <div className="parchment-card rounded-lg p-2.5 text-center">
              <KeyRound className="w-4 h-4 sm:w-5 sm:h-5 mx-auto mb-1 text-amber-800/70" strokeWidth={1.5} />
              <p className="text-[8px] sm:text-[9px] font-heading tracking-wider text-amber-900/70 mb-1.5">JOIN BY CODE</p>
              <Input
                value={joinCode}
                onChange={(e) => setJoinCode(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Enter') handleJoin(); }}
                placeholder="CODE"
                className="bg-amber-50/50 font-heading tracking-wider uppercase text-center text-[10px] h-7 border-amber-700/30 text-amber-900 placeholder:text-amber-700/30"
                maxLength={6}
              />
              <Button
                onClick={handleJoin}
                disabled={joining || !joinCode.trim()}
                className="w-full mt-1.5 h-7 text-[9px] bg-amber-800/80 text-amber-50 hover:bg-amber-800"
              >
                {joining ? <Loader2 className="w-3 h-3 animate-spin" /> : 'JOIN'}
              </Button>
            </div>
            {/* Create New Campaign */}
            <Link to="/games" className="parchment-card rounded-lg p-2.5 text-center flex flex-col items-center justify-start hover:brightness-105 transition-all">
              <Plus className="w-4 h-4 sm:w-5 sm:h-5 mx-auto mb-1 text-amber-800/70" strokeWidth={1.5} />
              <p className="text-[8px] sm:text-[9px] font-heading tracking-wider text-amber-900/70 leading-tight">CREATE NEW<br />CAMPAIGN</p>
            </Link>
            {/* Browse Adventures */}
            <Link to="/modules" className="parchment-card rounded-lg p-2.5 text-center flex flex-col items-center justify-start hover:brightness-105 transition-all">
              <Library className="w-4 h-4 sm:w-5 sm:h-5 mx-auto mb-1 text-amber-800/70" strokeWidth={1.5} />
              <p className="text-[8px] sm:text-[9px] font-heading tracking-wider text-amber-900/70 leading-tight">BROWSE<br />ADVENTURES</p>
            </Link>
          </div>
          ...
          {campaigns.length === 0 ? (
            <p className="font-tome italic text-xs text-amber-900/40 text-center py-3">No chronicles yet.</p>
          ) : (
            <div>
              {campaigns.map((c) => {
                const sys = getGameSystem(c.game_system || 'add1e');
                return (
                  <Link key={c.id} to={`/campaign/${c.id}`} className="flex items-center gap-2.5 py-2 border-b border-amber-800/15 hover:bg-amber-700/5 transition-colors group">
                    <div className="w-9 h-9 rounded overflow-hidden shrink-0 border border-amber-700/25 bg-amber-800/20">
                      <img src={thumbFor(c.game_system)} className="w-full h-full object-cover" alt="" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="font-heading font-600 text-sm text-amber-950 group-hover:text-amber-800 transition-colors truncate">{c.name}</p>
                      <p className="text-[10px] text-amber-900/45 font-body truncate">{sys.short} · Ch.{c.current_chapter} · {c.party_count} {c.party_count === 1 ? 'hero' : 'heroes'}</p>
                    </div>
                    ...
                    <span className="hidden sm:block text-[10px] text-amber-900/35 font-body whitespace-nowrap w-20 text-right">{moment(c.updated_date).fromNow()}</span>
                    <Settings className="w-3.5 h-3.5 text-amber-800/25 shrink-0" strokeWidth={1.5} />
                    {!c.is_owner && <Lock className="w-3.5 h-3.5 text-amber-800/25 shrink-0" strokeWidth={1.5} />}
                  </Link>
                );
              })}
            </div>
            ...
            <>
              ...
              <div>
                {characters.map((ch) => {
                  ...
                  return (
                    <Link key={ch.id} to={`/campaign/${ch.campaign_id}/character/${ch.id}`} className={`flex items-center gap-2.5 py-2 border-b border-amber-800/15 hover:bg-amber-700/5 transition-colors group ${dead ? 'opacity-50' : ''}`}>
                      <div className="w-9 h-9 rounded-full bg-amber-200/40 border border-amber-700/25 flex items-center justify-center shrink-0">
                        <ScrollText className="w-4 h-4 text-amber-800/60" strokeWidth={1.5} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="font-heading font-600 text-sm text-amber-950 group-hover:text-amber-800 transition-colors truncate">{ch.name}</p>
                        <p className="text-[10px] text-amber-900/45 font-body truncate">{ch.race} {ch.character_class} · Lvl {ch.level}</p>
                      </div>
                      {dead ? (
                        <span className="text-[8px] sm:text-[9px] font-heading tracking-wider px-1.5 py-0.5 rounded border bg-red-200/50 text-red-800 border-red-600/30">FALLEN</span>
                      ) : (
                        <span className="text-[8px] sm:text-[9px] font-heading tracking-wider px-1.5 py-0.5 rounded border bg-amber-200/40 text-amber-800/60 border-amber-700/25">{sys.short}</span>
                      )}
                    </Link>
                  );
                })}
          ...
          {invites.length > 0 && (
            <>
              ...
              <div>
                {invites.map((c) => (
                  <button key={c.id} onClick={() => navigate(`/campaign/${c.id}/create-character`)} className="w-full flex items-center gap-2.5 py-2 border-b border-amber-800/15 hover:bg-amber-700/5 transition-colors group text-left">
                    <div className="w-9 h-9 rounded-full bg-amber-200/40 border border-amber-700/25 flex items-center justify-center shrink-0">
                      <KeyRound className="w-4 h-4 text-amber-800/60" strokeWidth={1.5} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="font-heading font-600 text-sm text-amber-950 group-hover:text-amber-800 transition-colors truncate">{c.name}</p>
                      <p className="text-[10px] text-amber-900/45 font-body truncate">{c.is_owner ? 'Awaiting your hero' : 'Join the party'}</p>
                    </div>
                    <span className="text-[10px] font-heading tracking-wide text-amber-700/60">JOIN</span>
                  </button>
                ))}
              </div>
            </>
          ...
          <div className="flex gap-2 mt-3 flex-wrap justify-center">
            <Link to="/games"><Button variant="outline" size="sm" className="border-amber-700/30 text-amber-800 hover:bg-amber-700/10"><Plus className="w-3.5 h-3.5 mr-1" /> Choose a Realm</Button></Link>
            <Link to="/modules"><Button variant="ghost" size="sm" className="text-amber-800/50 hover:text-amber-800"><Library className="w-3.5 h-3.5 mr-1" /> Library</Button></Link>
            <Link to="/how-to-use"><Button variant="ghost" size="sm" className="text-amber-800/50 hover:text-amber-800"><BookOpen className="w-3.5 h-3.5 mr-1" /> How to Use</Button></Link>
          </div>
        </div>
      </div>
    </div>
...
function SectionHeader({ title, className = '' }) {
  return (
    <div className={`flex items-center gap-3 mb-1 ${className}`}>
      ...
      <h3 className="font-heading text-[11px] tracking-[0.2em] text-amber-800/60">{title}</h3>
...
function StepRow({ icon: Icon, title, desc }) {
  return (
    <div className="flex items-start gap-2.5 py-1.5">
      ...
      <div>
        <p className="font-heading font-600 text-sm text-amber-950">{title}</p>
        <p className="text-[11px] text-amber-900/45 font-body leading-relaxed">{desc}</p>
      </div>
    </div>
  );
}