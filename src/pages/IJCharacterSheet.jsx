...
import {
  ChevronLeft, Heart, Compass, Coins, Package, Activity,
  Loader2, BookOpen, Download, Zap
} from 'lucide-react';
import { ABILITIES, WEAPON_SKILLS, getInitiativeMod, getBackboneMod, bestWeaponSkillLevel } from '@/lib/ijRules';
import ExportCharacterButton from '@/components/ExportCharacterButton';
import { toast } from 'sonner';

export default function IJCharacterSheet({ character: initialCharacter, campaignId }) {
  ...
  if (!character) {
    return <div className="text-center py-20 text-muted-foreground">Adventurer not found.</div>;
  }
  ...
  const bckMod = getBackboneMod(scores);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="text-right flex flex-col items-end gap-2">
            ...
            {character.status === 'active' && character.hp_current < character.hp_max && (
              ...
              >
                {resting ? <Loader2 className="w-3 h-3 animate-spin inline" /> : '⟳ Rest to Full Vitality'}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <StatBox icon={Heart} label="VITALITY" value={`${character.hp_current}/${character.hp_max}`} sub="Damage Capacity" accent="red" />
        <StatBox icon={Zap} label="INIT" value={`${initMod >= 0 ? '+' : ''}${initMod}`} sub="MOV / 10" />
        <StatBox icon={Compass} label="BACKBONE" value={`${bckMod >= 0 ? '+' : ''}${bckMod}%`} sub="Nerve Modifier" />
        <StatBox icon={Coins} label="DOLLARS" value={character.gold || 0} sub="Currency" />
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        ...
        <div className="border border-border/50 rounded-lg bg-card/40 p-4">
          ...
          <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-border/30">
            ...
            <span className="text-xs font-heading text-foreground">{character.gold || 0} dollars</span>
          </div>
        </div>
