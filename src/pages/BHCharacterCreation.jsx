...
import {
  BACKGROUNDS, ABILITIES, WEAPON_SKILLS, WORK_SKILLS,
  rollAbilityScores, computeHP, getBraveryMod, getInitiativeMod,
  skillCountFor, bestWeaponSkillLevel
} from '@/lib/bhRules';
import { Dices, ChevronLeft, ChevronRight, Check, Loader2, Crosshair, Sparkles, FileText } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { toast } from 'sonner';
import ImportCharacterSheetForm from '@/components/ImportCharacterSheetForm';

const STEPS = ['Background', 'Attributes', 'Skills', 'Identity', 'Review'];

function rollDie(sides) { return Math.floor(Math.random() * sides) + 1; }

export default function BHCharacterCreation() {
  ...
  if (importMode) {
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        ...
        >
          <ChevronLeft className="w-3.5 h-3.5" /> Back to Gunslinger Builder
        </button>
  ...
  const bg = background ? BACKGROUNDS[background] : null;
  const totalSkills = rawScores ? skillCountFor(rawScores) : 4;
  const baseSkillCount = bg ? bg.weaponSkills.length + bg.workSkills.length : 0;
  ...
  async function handleCreate() {
    ...
    try {
      ...
      toast.success(`${name} rides into town!`);
  ...
  const brvMod = rawScores ? getBraveryMod(rawScores) : 0;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="font-heading font-700 text-lg text-foreground tracking-wide">CREATE GUNSLINGER · BOOT HILL</h1>
      </div>
      ...
      <div className="border border-border/50 rounded-lg bg-card/40 panel-glow p-6 sm:p-8 min-h-[320px]">
        ...
        {step === 0 && (
          <div className="animate-ink">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-heading text-sm tracking-[0.15em] text-foreground">CHOOSE YOUR BACKGROUND</h2>
            </div>
            <div className="grid sm:grid-cols-2 gap-2.5">
              {Object.entries(BACKGROUNDS).map(([b, data]) => (
                <button
                  ...
                  onClick={() => {
                    setBackground(b);
                    const bData = BACKGROUNDS[b];
                    setBaseSkills([
                      ...bData.weaponSkills.map(s => ({ name: s, level: 1 })),
                      ...bData.workSkills.map(s => ({ name: s, level: rollDie(100) }))
                    ]);
                    setExtraSkills([]);
                  }}
                  className={`text-left p-3.5 rounded-lg border transition-all ${
                    background === b ? 'border-primary bg-primary/10' : 'border-border/40 hover:border-primary/40 bg-card/30'
                  }`}
                >
                  <p className="font-heading font-600 text-sm text-foreground">{b}</p>
        ...
        {step === 1 && (
          <div className="animate-ink">
            ...
            <p className="text-[11px] text-muted-foreground font-body mb-4">
              Boot Hill uses six percentile attributes (1-100). Roll d100 for each. Your Strength is your damage capacity — how much lead you can take.
            </p>
            {!scoresRolled ? (
              <button
        ...
        {step === 2 && (
          <div className="animate-ink">
            ...
            <p className="text-[11px] text-muted-foreground font-body mb-4">
              Your background grants starting skills. Your attribute total grants {bonusSlots} additional skill{bonusSlots === 1 ? '' : 's'} to pick from the work skills below — the cunning compensate for weakness.
            </p>

            <div className="space-y-3">
              {/* Background + chosen skills */}
              <div className="space-y-1.5">
                ...
                {baseWorkSkills.map((s, i) => (
                  <div key={`bw${i}`} className="flex items-center justify-between p-2 rounded border border-sky-900/30 bg-sky-950/10">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[9px] font-heading tracking-wide px-1.5 py-0.5 rounded bg-sky-900/30 text-sky-400">WORK</span>
                      <span className="text-xs font-heading font-600 text-foreground">{s.name}</span>
                    </div>
              ...
              {/* Pick additional work skills */}
              {extraSkills.length < bonusSlots && (
                <div>
                  <p className="text-[10px] font-heading tracking-wide text-muted-foreground/70 mb-2">
                    PICK {bonusSlots - extraSkills.length} MORE WORK SKILL{bonusSlots - extraSkills.length === 1 ? '' : 'S'}
                  </p>
          ...
          <div className="animate-ink space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="e.g. Texas Red, Doc Holliday"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="A lean drifter with a scarred jaw, black hat pulled low, two Colts slung low on the hip..."
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Rode out of Abilene after the trouble with the cattle baron, looking for a fresh start and a quiet town..."
          ...
          <div className="animate-ink">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="p-3 rounded-lg bg-secondary/30 border border-border/40">
                  <p className="text-[10px] font-heading tracking-wide text-muted-foreground">BACKGROUND</p>
                  <p className="font-heading font-600 text-foreground mt-0.5">{background}</p>
                </div>
              </div>
              <div className="p-3 rounded-lg bg-secondary/30 border border-border/40">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 rounded-lg bg-secondary/30 border border-border/40 text-center">
                  <p className="text-[10px] font-heading tracking-wide text-muted-foreground">GRIT (HP)</p>
                ...
                <div className="p-3 rounded-lg bg-secondary/30 border border-border/40 text-center">
                  <p className="text-[10px] font-heading tracking-wide text-muted-foreground">BRAVERY</p>
                  <p className="font-heading font-700 text-lg text-foreground">{brvMod >= 0 ? '+' : ''}{brvMod}%</p>
                </div>
              </div>
              <div className="p-3 rounded-lg bg-secondary/30 border border-border/40">
              ...
              <div className="p-3 rounded-lg bg-secondary/30 border border-border/40">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-background/40 text-primary font-heading">{bg?.dollars || 0} dollars</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="flex justify-between mt-5">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <Button
            ...
            className="bg-primary text-primary-foreground hover:bg-primary/90"
          >
            {creating ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Crosshair className="w-4 h-4 mr-1.5" /> Ride into Town</>}
          </Button>
        )}
      </div>
    </div>
