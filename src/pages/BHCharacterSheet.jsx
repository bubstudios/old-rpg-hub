...
import {
  ChevronLeft, Heart, Crosshair, Coins, Package, Activity,
...
export default function BHCharacterSheet({ character: initialCharacter, campaignId }) {
  ...
  if (!character) {
    return <div className="text-center py-20 text-muted-foreground">Gunslinger not found.</div>;
  }
  ...
  const brvMod = getBraveryMod(scores);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="text-right flex flex-col items-end gap-2">
            ...
            {character.status === 'active' && character.hp_current < character.hp_max && (
              ...
              >
                {resting ? <Loader2 className="w-3 h-3 animate-spin inline" /> : '⟳ Rest to Full Grit'}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <StatBox icon={Heart} label="GRIT" value={`${character.hp_current}/${character.hp_max}`} sub="Damage Capacity" accent="red" />
        <StatBox icon={Zap} label="INIT" value={`${initMod >= 0 ? '+' : ''}${initMod}`} sub="SPD / 10" />
        <StatBox icon={Crosshair} label="BRAVERY" value={`${brvMod >= 0 ? '+' : ''}${brvMod}%`} sub="Aim Modifier" />
            ...
            <div className="space-y-1.5 max-h-52 overflow-y-auto scrollbar-thin pr-1">
              ...
              {workSkills.map((s, i) => (
                <div key={`k${i}`} className="flex items-center justify-between p-1.5 rounded border border-sky-900/30 bg-sky-950/5">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[8px] font-heading tracking-wide px-1 py-0.5 rounded bg-sky-900/30 text-sky-400">WORK</span>
                    <span className="text-[11px] font-heading font-600 text-foreground">{s.name}</span>
                  </div>
