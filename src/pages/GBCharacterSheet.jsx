...
import {
  ChevronLeft, Heart, Ghost, Package, Activity, Coins,
  Loader2, BookOpen, Download, Star
} from 'lucide-react';
import { ABILITIES, SKILLS_BY_ATTR } from '@/lib/gbRules';
import ExportCharacterButton from '@/components/ExportCharacterButton';
import { toast } from 'sonner';

export default function GBCharacterSheet({ character: initialCharacter, campaignId }) {
  ...
  async function handleRest() {
    ...
    try {
      ...
      toast.success(`${character.name} recovers all Brownie Points.`);
  ...
  if (!character) {
    return <div className="text-center py-20 text-muted-foreground">Buster not found.</div>;
  }

  const scores = character.ability_scores || {};
  const skills = character.skills || [];
  const equipment = character.equipment || [];
  const bpPct = character.hp_max > 0 ? (character.hp_current / character.hp_max) * 100 : 0;

  function skillAttr(skillName) {
    for (const [attr, skillList] of Object.entries(SKILLS_BY_ATTR)) {
      if (skillList.includes(skillName)) return attr;
    }
    return null;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      <button onClick={() => navigate(`/campaign/${campaignId}`)} className="flex items-center gap-1.5 text-xs font-heading tracking-wide text-muted-foreground hover:text-foreground mb-6 transition-colors">
          ...
          <div className="text-right flex flex-col items-end gap-2">
            ...
            {character.status === 'active' && character.hp_current < character.hp_max && (
              <button onClick={handleRest} disabled={resting} className="block mt-2 text-[10px] font-heading tracking-wide text-primary hover:text-primary/80 transition-colors">
                {resting ? <Loader2 className="w-3 h-3 animate-spin inline" /> : '⟳ Recover Brownie Points'}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <StatBox icon={Star} label="BROWNIE PTS" value={`${character.hp_current}/${character.hp_max}`} sub="Hero Points" accent="red" />
        <StatBox icon={Ghost} label="BUSTERS" value={character.race} sub="Archetype" />
        <StatBox icon={Activity} label="TAG SKILLS" value={skills.length} sub="Bonus Dice" />
        <StatBox icon={Coins} label="BP BANK" value={character.gold || 0} sub="Currency" />
      </div>

      <div className="mb-5">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[10px] font-heading tracking-[0.15em] text-muted-foreground">BROWNIE POINTS</span>
        ...
        <div className="h-2.5 rounded-full bg-secondary overflow-hidden border border-border/40">
          <div className={`h-full rounded-full transition-all duration-500 ${bpPct > 60 ? 'bg-emerald-700' : bpPct > 30 ? 'bg-amber-700' : 'bg-red-800'}`} style={{ width: `${Math.max(0, bpPct)}%` }} />
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <div className="border border-border/50 rounded-lg bg-card/40 p-4">
          <div className="flex items-center gap-2 mb-3">
            <Ghost className="w-3.5 h-3.5 text-primary" strokeWidth={1.5} />
            ...
            {ABILITIES.map((a) => (
              <div key={a.key} className="flex items-center justify-between p-2 rounded-lg bg-secondary/30 border border-border/30">
                <div>
                  ...
                  <p className="font-heading font-700 text-lg text-foreground tabular-nums">{scores[a.key] ?? 3}<span className="text-[10px] text-muted-foreground">d6</span></p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="border border-border/50 rounded-lg bg-card/40 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-heading text-[11px] tracking-[0.15em] text-foreground">TAG SKILLS</h3>
          </div>
          {skills.length ? (
            <div className="space-y-1.5 max-h-52 overflow-y-auto scrollbar-thin pr-1">
              {skills.map((s, i) => (
                <div key={i} className="flex items-center justify-between p-1.5 rounded border border-sky-900/30 bg-sky-950/5">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[8px] font-heading tracking-wide px-1 py-0.5 rounded bg-sky-900/30 text-sky-400 uppercase">{skillAttr(s.name) || '—'}</span>
