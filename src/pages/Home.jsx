    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
      ...
      <div className="text-center mb-12">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full wax-seal mb-5 animate-flicker ring-2 ring-[#3a0808] candle-glow">
          <ScrollText className="w-7 h-7 text-amber-50" strokeWidth={1.2} />
        </div>
        <h1 className="font-heading font-900 text-3xl sm:text-5xl tracking-[0.08em] text-[#d4af37] mb-3">
          {game.dashboardTitle}
        </h1>
        <p className="font-tome text-base sm:text-lg text-[#e5d3b3]/50 italic max-w-xl mx-auto leading-relaxed">
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-12">
        <div className="cathedral-panel p-5">
          <div className="flex items-center gap-2 mb-3">
            <Plus className="w-4 h-4 text-[#d4af37]" strokeWidth={1.5} />
            <h2 className="font-heading text-sm tracking-[0.15em] text-[#e5d3b3]">BEGIN A CAMPAIGN</h2>
          </div>
          {showCreate ? (
            ...
            <div className="space-y-2">
              <Button onClick={() => setShowCreate(true)} variant="outline" className="w-full border-[#d4af37]/30 text-[#d4af37] hover:bg-[#d4af37]/10">
                <Settings2 className="w-4 h-4 mr-1.5" /> Create New Campaign
              </Button>
              <Button onClick={() => setShowImport(true)} variant="ghost" className="w-full border-[#d4af37]/15 text-[#e5d3b3]/50 hover:text-[#d4af37]">
        ...
        <div className="cathedral-panel p-5">
          <div className="flex items-center gap-2 mb-3">
            <KeyRound className="w-4 h-4 text-[#d4af37]" strokeWidth={1.5} />
            <h2 className="font-heading text-sm tracking-[0.15em] text-[#e5d3b3]">JOIN BY CODE</h2>
          </div>
          <p className="text-xs text-[#e5d3b3]/40 font-body mb-3 leading-relaxed">
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="bg-[#0d0a08] border-[#d4af37]/25 text-[#e5d3b3] font-heading tracking-wider uppercase placeholder:text-[#e5d3b3]/25"
              maxLength={6}
            />
            <Button onClick={handleJoin} disabled={joining || !joinCode.trim()} className="bg-gradient-to-b from-[#d4af37] to-[#8a6a1f] text-[#0d0a08] hover:from-[#f0c66d] hover:to-[#d4af37]">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-heading text-sm tracking-[0.15em] text-[#e5d3b3]">YOUR CAMPAIGNS</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 gothic-inset rounded-lg">
            <ScrollText className="w-8 h-8 text-[#d4af37]/20 mx-auto mb-3" strokeWidth={1} />
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {campaigns.map((c) => (
              <div
                key={c.id}
                className="group relative text-left gothic-inset p-4 hover:border-[#d4af37]/40 transition-all rounded-lg"
              >
                <button
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <h3 className="font-heading font-600 text-base text-[#e5d3b3] group-hover:text-[#d4af37] transition-colors">
                      {c.name}
                    </h3>
                    <ChevronRight className="w-4 h-4 text-[#e5d3b3]/30 group-hover:text-[#d4af37] transition-colors shrink-0 mt-0.5" />
                  </div>
                  <div className="flex items-center gap-3 flex-wrap text-[11px] text-[#e5d3b3]/40 font-body">
                    ...
                    <span className={`px-1.5 py-0.5 rounded font-heading tracking-wide ${
                      c.status === 'active' ? 'bg-emerald-900/20 text-emerald-400' :
                      c.status === 'setup' ? 'bg-amber-900/20 text-amber-400' :
                      'bg-[#d4af37]/5 text-[#e5d3b3]/40'
                    }`}>
                  ...
                  <div className="mt-2.5 flex items-center justify-between">
                    <span className="text-[10px] font-heading tracking-[0.15em] text-[#e5d3b3]/30">
                      CODE: {c.invite_code}
                    </span>
                    {!c.has_character && (
                ...
                {c.is_owner && (
                  <button
                    ...
                    className="absolute top-2 right-2 p-1.5 rounded text-[#e5d3b3]/30 hover:text-red-400 hover:bg-red-900/10 transition-colors disabled:opacity-50"
                  >
