  ...
  return (
    <div className="space-y-3">
      <div className="border border-border/50 rounded-lg bg-card/40 p-4">
        ...
        {items.length === 0 ? (
          <p className="text-xs text-muted-foreground/50 font-body">Bullet carries nothing but the clothes on his back and the shard in his pocket.</p>
        ) : (
          <div className="space-y-3">
            {items.map((item, i) => {
                  ...
                  {detail && (
                    <div className="mt-1.5 space-y-1.5">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {detail.uses.map(use => (
                          <span key={use} className="text-[11px] px-1.5 py-0.5 rounded bg-secondary/40 text-muted-foreground font-body border border-border/30">
                            {use}
                          </span>
                        ))}
                      </div>
