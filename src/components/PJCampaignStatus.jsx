...
import { Rocket, Shield, Activity, AlertTriangle, Users, FileText, MapPin, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="border border-border/50 rounded-lg bg-card/40 panel-glow p-3 space-y-3">
            ...
            return (
              <div key={c.key}>
                <div className="flex items-center justify-between text-[10px] mb-0.5">
                  <span className="text-muted-foreground font-body truncate" title={c.desc}>{c.label}</span>
        ...
        {showEvidence && (
          <div className="space-y-0.5">
            {evidence.length ? evidence.map((e, i) => (
              <p key={i} className="text-[10px] text-muted-foreground font-body leading-snug">• {e}</p>
      ...
      <div className="grid grid-cols-2 gap-2 pt-1 border-t border-border/30">
        <div>
          ...
          {allies.length ? allies.map((a, i) => <p key={i} className="text-[9px] text-muted-foreground font-body leading-snug">• {a}</p>) : <p className="text-[9px] text-muted-foreground/50 italic">None</p>}
        </div>
        <div>
          <div className="flex items-center gap-1 mb-1">
          ...
          {enemies.length ? enemies.map((e, i) => <p key={i} className="text-[9px] text-muted-foreground font-body leading-snug">• {e}</p>) : <p className="text-[9px] text-muted-foreground/50 italic">None</p>}
        </div>
      </div>
    </div>
  );
