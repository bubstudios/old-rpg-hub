...
import { Dices, Swords, Activity, Zap, X, Rocket } from 'lucide-react';
import { toast } from 'sonner';

const DICE = [4, 6, 8, 10, 12, 20, 100];
const SF_ABILITIES = [
  { key: 'str', label: 'STR' }, { key: 'int', label: 'INT' }, { key: 'log', label: 'LOG' }, { key: 'dex', label: 'DEX' },
  { key: 'rs', label: 'RS' }, { key: 'per', label: 'PER' }, { key: 'ldr', label: 'LDR' }, { key: 'sta', label: 'STA' }
];

export default function SFDiceRollerPanel({ myCharacter, campaignId, chapter, onRolled, onClose }) {
  ...
  async function roll(payload) {
    setRolling(true);
    try {
      const res = await base44.functions.invoke('rollDice', { ...payload, game_system: 'starfrontiers' });
      onRolled?.(res.data);
    } catch (e) {
    ...
    <div className="mb-3 rounded-lg border border-border/50 bg-card/60 p-3 animate-ink">
      <div className="flex items-center gap-1 mb-3">
        <span className="px-2 py-1 rounded text-[11px] font-heading tracking-wide bg-primary/15 text-primary flex items-center gap-1">
          <Rocket className="w-3 h-3" /> Star Frontiers
        </span>
      ...
      {rollType === 'attack' && (
        <div className="space-y-2">
          <div className="flex gap-1.5">
            <ToggleBtn active={ranged} onClick={() => setRanged(true)} label="Ranged (DEX)" />
            <ToggleBtn active={!ranged} onClick={() => setRanged(false)} label="Melee (STR)" />
          </div>
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Roll d100 — hit if ≤ (ability ÷ 2 + 10 per skill level).</p>
        </div>
      )}
      ...
      {rollType === 'ability' && (
        <div className="space-y-2">
          ...
          <Button onClick={() => rollAbility(ability)} disabled={rolling} size="sm" className="w-full bg-primary text-primary-foreground">
            <Dices className="w-3.5 h-3.5" /> Roll {SF_ABILITIES.find(a => a.key === ability)?.label} (d100)
          </Button>
          <p className="text-[10px] text-muted-foreground/60 font-body">Roll d100 — success if ≤ ability score.</p>
        </div>
      )}
      ...
      {rollType === 'initiative' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Initiative = RS ÷ 10 + d10. Higher acts first.</p>
        </div>
      )}
