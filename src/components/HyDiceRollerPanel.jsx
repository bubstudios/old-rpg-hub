...
import { Dices, Flame, Activity, Zap, X, Heart, Skull } from 'lucide-react';
import { toast } from 'sonner';

const DICE = [4, 6, 8, 10, 12, 20, 100];
const HY_ABILITIES = [
  { key: 'str', label: 'STR' }, { key: 'dex', label: 'DEX' }, { key: 'agi', label: 'AGI' },
  { key: 'end', label: 'END' }, { key: 'sta', label: 'STA' }, { key: 'int', label: 'INT' },
  { key: 'men', label: 'MEN' }, { key: 'lck', label: 'LCK' }
];

export default function HyDiceRollerPanel({ myCharacter, campaignId, chapter, onRolled, onClose, gameSystem = 'conan' }) {
  ...
  async function roll(payload) {
    setRolling(true);
    try {
      const res = await base44.functions.invoke('rollDice', { ...payload, game_system: gameSystem });
      onRolled?.(res.data);
    } catch (e) {
    ...
    <div className="mb-3 rounded-lg border border-border/50 bg-card/60 p-3 animate-ink">
      <div className="flex items-center gap-1 mb-3">
        <span className="px-2 py-1 rounded text-[11px] font-heading tracking-wide bg-primary/15 text-primary flex items-center gap-1">
          <Flame className="w-3 h-3" /> {gameSystem === 'redsonja' ? 'Red Sonja' : 'Conan'}
        </span>
      ...
      <div className="grid grid-cols-6 gap-1.5 mb-3">
        <RollTypeBtn active={rollType === 'attack'} onClick={() => setRollType('attack')} icon={Flame} label="Attack" />
      ...
      {rollType === 'attack' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Roll d100 — hit if you roll ≤ your hit number (Dexterity + Mentation mod + weapon skill + modifier). Add range/cover/movement penalties here.</p>
        </div>
      )}
      ...
      {rollType === 'wound' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Rolls d100 for wound location (body part) and d100 for severity (Slight → Mortal). Damage is subtracted from the target's Vitality (Endurance).</p>
        </div>
      )}
      ...
      {rollType === 'ability' && (
        <div className="space-y-2">
          ...
          <Button onClick={() => rollAbility(ability)} disabled={rolling} size="sm" className="w-full bg-primary text-primary-foreground">
            <Dices className="w-3.5 h-3.5" /> Roll {HY_ABILITIES.find(a => a.key === ability)?.label} (d100)
          </Button>
      ...
      {rollType === 'initiative' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Initiative = d10 + AGI/10. Higher acts first.</p>
        </div>
      )}
      ...
      {rollType === 'reaction' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Roll d100 — beat your Agility to dodge, evade, or react in time.</p>
        </div>
      )}
