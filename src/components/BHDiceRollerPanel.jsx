...
import { Dices, Crosshair, Activity, Zap, X, Heart, Skull } from 'lucide-react';
import { toast } from 'sonner';

const DICE = [4, 6, 8, 10, 12, 20, 100];
const BH_ABILITIES = [
  { key: 'spd', label: 'SPD' }, { key: 'gacc', label: 'GACC' }, { key: 'tacc', label: 'TACC' },
  { key: 'str', label: 'STR' }, { key: 'brv', label: 'BRV' }, { key: 'exp', label: 'EXP' }
];

export default function BHDiceRollerPanel({ myCharacter, campaignId, chapter, onRolled, onClose }) {
  const [rollType, setRollType] = useState('attack');
  const [firearm, setFirearm] = useState(true);
  const [attackMod, setAttackMod] = useState(0);
  const [ability, setAbility] = useState('gacc');
  ...
  async function roll(payload) {
    setRolling(true);
    try {
      const res = await base44.functions.invoke('rollDice', { ...payload, game_system: 'boothill' });
      onRolled?.(res.data);
    } catch (e) {
  ...
  const rollAttack = () => roll({ ...base, roll_type: 'attack', firearm, modifier: attackMod });
  ...
  const rollQuickDraw = () => roll({ ...base, roll_type: 'quickdraw' });
    ...
    <div className="mb-3 rounded-lg border border-border/50 bg-card/60 p-3 animate-ink">
      <div className="flex items-center gap-1 mb-3">
        <span className="px-2 py-1 rounded text-[11px] font-heading tracking-wide bg-primary/15 text-primary flex items-center gap-1">
          <Crosshair className="w-3 h-3" /> Boot Hill
        </span>
      ...
      <div className="grid grid-cols-6 gap-1.5 mb-3">
        ...
        <RollTypeBtn active={rollType === 'quickdraw'} onClick={() => setRollType('quickdraw')} icon={Heart} label="Draw" />
      ...
      {rollType === 'attack' && (
        <div className="space-y-2">
          <div className="flex gap-1.5">
            <ToggleBtn active={firearm} onClick={() => setFirearm(true)} label="Firearm (GACC)" />
            <ToggleBtn active={!firearm} onClick={() => setFirearm(false)} label="Thrown (TACC)" />
          </div>
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Roll d100 — hit if you roll ≤ your hit number (base accuracy + Bravery mod + weapon skill + modifier). Add range/cover/movement penalties here.</p>
        </div>
      )}
      ...
      {rollType === 'wound' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Rolls d100 for wound location (body part) and d100 for severity (Slight → Mortal). Damage is subtracted from the target's Grit (Strength).</p>
        </div>
      )}
      ...
      {rollType === 'ability' && (
        <div className="space-y-2">
          ...
          <Button onClick={() => rollAbility(ability)} disabled={rolling} size="sm" className="w-full bg-primary text-primary-foreground">
            <Dices className="w-3.5 h-3.5" /> Roll {BH_ABILITIES.find(a => a.key === ability)?.label} (d100)
          </Button>
      ...
      {rollType === 'initiative' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Initiative = d10 + SPD/10. Higher acts first.</p>
        </div>
      )}

      {rollType === 'quickdraw' && (
        <div className="space-y-2">
          <Button onClick={rollQuickDraw} disabled={rolling} size="sm" className="w-full bg-primary text-primary-foreground">
            <Dices className="w-3.5 h-3.5" /> Roll Quick Draw (d100)
          </Button>
          <p className="text-[10px] text-muted-foreground/60 font-body">Roll d100 — beat your opponent by rolling under your Speed. Lowest roll wins a tie.</p>
        </div>
      )}
