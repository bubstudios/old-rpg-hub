...
const GANG_ABILITIES = [
  { key: 'mus', label: 'MUS' }, { key: 'agi', label: 'AGI' }, { key: 'aim', label: 'AIM' },
  { key: 'sav', label: 'SAV' }, { key: 'ner', label: 'NER' }, { key: 'pan', label: 'PAN' }
];

export default function GangDiceRollerPanel({ myCharacter, campaignId, chapter, onRolled, onClose }) {
  const [rollType, setRollType] = useState('attack');
  const [attackMod, setAttackMod] = useState(0);
  const [ability, setAbility] = useState('aim');
  ...
  async function roll(payload) {
    setRolling(true);
    try {
      const res = await base44.functions.invoke('rollDice', { ...payload, game_system: 'gangbusters' });
      onRolled?.(res.data);
    } catch (e) {
    ...
    <div className="mb-3 rounded-lg border border-border/50 bg-card/60 p-3 animate-ink">
      <div className="flex items-center gap-1 mb-3">
        <span className="px-2 py-1 rounded text-[11px] font-heading tracking-wide bg-primary/15 text-primary flex items-center gap-1">
          <Crosshair className="w-3 h-3" /> Gangbusters
        </span>
      ...
      {rollType === 'attack' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Roll d100 — hit if you roll ≤ your hit number (Aim + Nerve mod + weapon skill + modifier). Add range/cover/movement penalties here.</p>
        </div>
      )}
      ...
      {rollType === 'wound' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Rolls d100 for wound location (body part) and d100 for severity (Slight → Mortal). Damage is subtracted from the target's Grit (Muscle).</p>
        </div>
      )}
      ...
      {rollType === 'ability' && (
        <div className="space-y-2">
          ...
          <Button onClick={() => rollAbility(ability)} disabled={rolling} size="sm" className="w-full bg-primary text-primary-foreground">
            <Dices className="w-3.5 h-3.5" /> Roll {GANG_ABILITIES.find(a => a.key === ability)?.label} (d100)
          </Button>
      ...
      {rollType === 'reaction' && (
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground/60 font-body">Roll d100 — beat your Agility to dodge, draw first, or react in time.</p>
        </div>
      )}
